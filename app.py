...
def run_research(query: str, status_placeholder=None) -> Optional[ResearchResponse]:
    ...
    # Set research mode from UI state (PRD §4.5.2)
        ...
        def on_tool_start(tool_name: str, args: dict):
            ...
            if status_placeholder:
                if tool_name == "web_search":
                    status_placeholder.markdown("🔍 **Searching web...**")
                elif tool_name == "fetch_page":
                    url = args.get("url", "")[:40]
                    status_placeholder.markdown(f"📄 **Reading sources...** `{url}...`")
                elif tool_name == "save_note":
                    status_placeholder.markdown("💾 **Saving note...**")
                elif tool_name == "list_notes":
                    status_placeholder.markdown("📝 **Searching notes...**")
                elif tool_name == "get_note":
                    status_placeholder.markdown("📝 **Retrieving note...**")
                else:
...
def render_sidebar():
    ...
    if st.button("🗑️ Clear History", help="Clear chat history"):
        clear_history()
        st.rerun()
    ...
    if search_provider:
        provider_display = "🔍 Serper API" if search_provider == "serper" else "🦆 DuckDuckGo"
        st.caption(f"Search: {provider_display}")

    if not services["ollama"]:
        st.caption("⚠️ Start Ollama to enable research")
    if not services["mcp"]:
        st.caption("⚠️ Start MCP server for tools")
...
def render_main_content():
    ...
    # Handle pending follow-up questions (PRD §5.10)
    ...
    # Research Trail panel (PRD §5.10 Demo Polish Pack)
    ...
    with chat_container:
        for i, msg in enumerate(st.session_state.get("messages", [])):
            ...
            show_save = role == "assistant" and content and can_save and not content.startswith("🚧")
            ...
            saved = render_chat_message(
                ...
                sources_title=f"📚 {len(sources)} Sources" if sources else "📚 Sources",
    ...
    # Render follow-up chips for the LAST assistant message (PRD §5.10)
    ...
    # Error display with recovery steps (PRD §6)
    if state.error:
    ...
    if prompt:
        ...
        if not services["ollama"]:
            ...
            add_message("assistant", "❌ Ollama is not available. Please start Ollama first.")
        elif not services["mcp"]:
            with chat_container:
                with st.chat_message("assistant"):
                    st.warning(
                        "⚠️ MCP server is not running. Research tools are unavailable.\n\n"
            ...
            # Run research
            with chat_container:
                # Use status placeholder for specific tool status (PRD §4.5.1)
                ...
                if response:
                    ...
                    # Add to message history with sources, metadata, tool traces, and follow-ups (PRD §5.10)
                    ...
                    add_message("assistant", "❌ Research failed. Please try again.")
                    st.rerun()
...
def main() -> None:
    ...
    # Custom CSS - Design tokens from PRD §7 & CLAUDE.md
    st.markdown("""
        <style>
        ...
        /* Citation superscript styling per PRD §5.10 */
        sup.citation a {
            color: #0066CC;
    ...
    tab_research, tab_about = st.tabs(["🔍 Research", "ℹ️ How It Works"])
